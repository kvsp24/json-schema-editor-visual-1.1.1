...
class jsonSchema extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      mock: '',
      paramListData:[{
        id:1,value:'poiId:[Long](门店列表)'
      },{
        id:2,value:'userId:Long(用户id)'
      },{
        id:3,value:'userNewActivity:Boolean(使用新活动)'
      },{
        id:4,value:'wmPoiId:Long(外卖门店id)'
      }]
    };
  ...
  */
  ...
  plantParamListData= (data) =>{
    const arr = [];
    if(data && data.length>0){
      data.map((item) => {
        arr.push({
          value: item.id,
          label: item.forDisplay,
        });
      });
    }
    return arr;
  ...
  fetchParamListData=()=>{
    Request.get('/adminserver/param/list').then((res)=>{
      const {data} = res;
      if(data===0){
        const arr = this.plantParamListData(data.udsParamVoList);
        this.setState({
          paramListData: arr,
        })
      }
